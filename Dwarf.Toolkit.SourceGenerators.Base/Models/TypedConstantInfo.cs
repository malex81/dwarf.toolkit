...
internal abstract partial record TypedConstantInfo
{
	...
	{
		public static Enum Create(ITypeSymbol typeSimb, object value)
		{
			var typeName = typeSimb.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
			if (value is int num)
			{
				var member = typeSimb.GetMembers().FirstOrDefault(m => m is IFieldSymbol field && field.ConstantValue is int _num && _num == num);
				if (member != null)
					return new(typeName, member.Name);
			}
